...
public class KeyEd25519 {
    ...
    private ASN1ObjectIdentifier getDERPrefix() {
        return new ASN1ObjectIdentifier("1.3.101.112");
    }
    ...
    */
    public void readPrivateKey(String filename) throws IOException {
        ...
        var keyInfo = PrivateKeyInfo.getInstance(derKey);
        String algoId = keyInfo.getPrivateKeyAlgorithm().getAlgorithm().toString();
        if (algoId.equals("1.3.101.112")) {
    ...
    public void writePrivateKey(String filename) throws IOException {
        var derPrefix = new DERSequence(getDERPrefix());
